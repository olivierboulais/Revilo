...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const scansUsedToday = user ? await countScansToday(user.id) : 0;
  const scansRemaining = Math.max(0, scanLimit - scansUsedToday);
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0 flex flex-col gap-4">
        <TopBar workspaceName={session.workspaceName} scannedAt={report.scannedAt} scansRemaining={scansRemaining} scansLimit={scanLimit} />
