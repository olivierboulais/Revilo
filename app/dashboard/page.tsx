...
export default async function DashboardPage() {
  ...
  // Run scan inline if no cached report exists — this handles the case where
  // KV isn't configured yet (Vercel free tier without a KV store attached),
  // or where the report expired. The scan is fast (<200ms on mock data).
    ...
    <main className="flex-1">
      <div className="max-w-[920px] mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
          <ScoreCard label="Alignment Score" value={report.alignment.overall} accent="primary"
            subscores={[
          ...
          <ScoreCard label="Adoption Score" value={report.adoption.overall} locked={!isPaid}
            subscores={[
          ...
          <ScoreCard label="Architecture Score" value={report.architecture.overall} locked={!isPaid}
            subscores={[
        ...
        <section className="mb-12">
          ...
          {isPaid ? (
            <div className="rounded-2xl border border-line bg-white px-5"><FindingsList findings={adoptionFindings} /></div>
          ) : (
        ...
        <section className="mb-12">
          ...
          {isPaid ? (
            <div className="rounded-2xl border border-line bg-white px-5"><FindingsList findings={architectureFindings} /></div>
          ) : (
        ...
        <section className="mb-12">
          ...
          {isPaid ? (
            <div className="rounded-2xl border border-line bg-white px-5"><TeamInsightsList insights={report.teamInsights} /></div>
          ) : (
        ...
        <section className="mb-12">
          ...
          {isPaid ? (
            <div className="flex flex-col gap-8">
              {quickWins.length > 0 && <div><h3 className="text-[13px] font-medium text-gray uppercase tracking-wide mb-3">Quick wins</h3><div className="flex flex-col gap-3">{quickWins.map((r) => <RecommendationCard key={r.id} rec={r} />)}</div></div>}
              {mediumTerm.length > 0 && <div><h3 className="text-[13px] font-medium text-gray uppercase tracking-wide mb-3">Medium-term improvements</h3><div className="flex flex-col gap-3">{mediumTerm.map((r) => <RecommendationCard key={r.id} rec={r} />)}</div></div>}
              {strategic.length > 0 && <div><h3 className="text-[13px] font-medium text-gray uppercase tracking-wide mb-3">Strategic improvements</h3><div className="flex flex-col gap-3">{strategic.map((r) => <RecommendationCard key={r.id} rec={r} />)}</div></div>}
            </div>
          ) : (
        ...
        {!isPaid && (
          <div className="rounded-2xl bg-[#1C1C1A] text-white p-8 flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
            ...
            <LinkButton href="/upgrade" variant="lilac">Unlock Full Report</LinkButton>
          </div>
        )}
      </div>
    </main>
